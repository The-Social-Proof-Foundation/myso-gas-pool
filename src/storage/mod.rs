...
#[cfg(test)]
pub async fn connect_storage_for_testing(sponsor_address: MySoAddress) -> Arc<dyn Storage> {
...
mod tests {
    ...
    use myso_types::base_types::{random_object_ref, ObjectID, SequenceNumber, MySoAddress};
    ...
    async fn setup(sponsor: MySoAddress, init_balances: Vec<u64>) -> Arc<dyn Storage> {
    ...
    async fn test_gas_pool_init() {
        let sponsor = MySoAddress::random_for_testing_only();
        let storage = connect_storage_for_testing(sponsor).await;
