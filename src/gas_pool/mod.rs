...
mod tests {
    ...
    async fn test_station_reserve_gas() {
        let (_test_cluster, container) =
            start_gas_station(vec![MIST_PER_MYSO; 10], MIST_PER_MYSO).await;
        let station = container.get_gas_pool_arc();
        let (sponsor1, _res_id1, gas_coins) = station
            .reserve_gas(MIST_PER_MYSO * 3, Duration::from_secs(10))
            .await
            .unwrap();
        ...
        let (sponsor2, _res_id2, gas_coins) = station
            .reserve_gas(MIST_PER_MYSO * 7, Duration::from_secs(10))
            .await
            .unwrap();
    ...
    async fn test_e2e_gas_station_flow() {
        let (test_cluster, container) = start_gas_station(vec![MIST_PER_MYSO], MIST_PER_MYSO).await;
        let station = container.get_gas_pool_arc();
        assert!(station
            .reserve_gas(MIST_PER_MYSO + 1, Duration::from_secs(10))
            .await
            .is_err());

        let (sponsor, reservation_id, gas_coins) = station
            .reserve_gas(MIST_PER_MYSO, Duration::from_secs(10))
            .await
            .unwrap();
    ...
    async fn test_invalid_transaction() {
        ...
        let (_test_cluster, container) = start_gas_station(vec![MIST_PER_MYSO], MIST_PER_MYSO).await;
        let station = container.get_gas_pool_arc();
        let (sponsor, reservation_id, gas_coins) = station
            .reserve_gas(MIST_PER_MYSO, Duration::from_secs(10))
            .await
        ...
        let user_sig = Signature::new_secure(
            &IntentMessage::new(Intent::myso_transaction(), &tx_data),
            &keypair,
        );
        let result = station
    ...
    async fn test_coin_expiration() {
        ...
        let station = container.get_gas_pool_arc();
        let (sponsor, reservation_id, gas_coins) = station
            .reserve_gas(MIST_PER_MYSO, Duration::from_secs(1))
            .await
            .unwrap();
        assert_eq!(gas_coins.len(), 1);
        assert_eq!(station.query_pool_available_coin_count().await, 0);
